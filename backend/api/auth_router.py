...
async def register(user_data: UserRegister):
    ...
    Default roles:
    - hr: HR / administrator users
    - recruiter: Standard user (default)
    - student: Candidate / student accounts
    """
    conn = None
    try:
...
async def get_current_user_info(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT id, username, email, role, bio FROM users WHERE id = %s",
...
async def update_profile(
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute(
                f"UPDATE users SET {', '.join(updates)} WHERE id = %s RETURNING id, username, email, role, bio",
                tuple(params),
            )
            user = cur.fetchone()
            conn.commit()
