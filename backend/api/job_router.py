...
def _send_shortlisted_resume_email(resume_data_id: int, candidate_name: str, candidate_email: str) -> None:
    ...
    try:
        from backend.api.email_router import EmailRequest, get_resume_shortlisted_email, send_email_via_smtp
...
async def batch_screen_resumes(
    ...
    try:
        ...
        try:
            ...
            if redis_client:
                channel = f"screening:{jobId}"
                update_msg = {
                    "type": "screening_update",
                    "status": "completed",
                    "job_id": jobId,
                    "processed_count": len(saved_candidates)
                }
                import json
                redis_client.publish(channel, json.dumps(update_msg))
                print(f"--> WS PUB/SUB: Published batch screen completion for job {jobId}")
        except Exception as ws_err:
            print(f"--> WS PUB/SUB ERROR: Failed to publish batch screen completion: {ws_err}")
