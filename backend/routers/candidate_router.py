...
def _require_candidate(current_user: dict):
    """Raise 403 if the caller is not a candidate (super_admin bypasses for testing)."""
    if current_user.get("role") not in ("candidate", "super_admin"):
        raise HTTPException(
...
async def get_my_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Resolve email from the users table using user_id stored in JWT
            ...
            # Fetch resume / application record
            cur.execute(
                """
                SELECT
                    id,
            ...
            # job_title: jobs table not yet in this DB, skip for now
            job_title = None


            # Fetch scheduled interview (most recent)
            cur.execute(
                """
                SELECT
            ...
            # Map interview_status to timeline
