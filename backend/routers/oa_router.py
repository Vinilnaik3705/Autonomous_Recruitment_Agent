...
def _normalize_submission(payload: Dict[str, Any]) -> OAResultSubmission:
    ...
    # Some tools post nested body payloads (e.g. { body: { ... } })
...
def _score_from_url_tokens(report_url: str) -> Optional[float]:
    ...
    # 1) Query-string based score: ?score=82, ?oa_score=82, ?result=82/100
    ...
    # 2) Path token based score: .../oa-82, .../score-82, .../result-82
...
def _resolve_score(submission: OAResultSubmission) -> float:
    ...
    if submission.report_url:
        if is_hackerrank_report:
            # Prefer authoritative HackerRank page score over callback query tokens.
    ...
    # Fallback only when URL-based extraction is unavailable.
...
def trigger_scheduling_workflow(candidate_email: str, candidate_name: str, score: float):
    ...
    try:
        ...
        # Keep compatibility with existing schedule workflow payload expectations.
        payload = {
            "job_id": "OA-AUTO",
...
def _process_oa_result(
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Validate candidate exists in main applicant table.
            cur.execute(
                """
            ...
            if cur.rowcount == 0:
                # If resume_data row was deleted, recreate a minimal record for this candidate.
                cur.execute(
                    """
            ...
            # Keep legacy candidates table in sync if email exists there.
            cur.execute(
                """
                UPDATE candidates
                SET oa_score = %s
