...
async def create_order(order: PaymentOrder):
    # When Razorpay SDK is not available (e.g. in local/demo mode), return a fake order
    # so the frontend flow continues without hard failures.
    if not client:
    ...
    try:
        # Amount is in paise for INR, cents for USD
        order_data = {
...
async def verify_payment(verification: PaymentVerification):
    # In local/demo mode without Razorpay client, treat all payments as successful
    # so users can exercise the full flow without real card details.
    if not client:
