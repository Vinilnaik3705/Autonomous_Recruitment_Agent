...
import {
  Users, Shield, RefreshCw, UserCheck, UserX,
  LogOut, Rocket, Clock, ClipboardList, Calendar,
  TrendingUp, Loader2, Check, Search, FileUp, Sparkles,
  Zap, ChevronRight, Upload, FileText, X, ChevronDown, Eye, LayoutDashboard
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  // Tabs layout update: default to screening, removed analytics dashboard tab
  const [activeTab, setActiveTab] = useState<"screening" | "interviews" | "onboarding" | "employees" | "settings">("screening");
  const [usersList, setUsersList] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Recruiter Screening State
  ...
  const [selectedOfferLetter, setSelectedOfferLetter] = useState<string>("");
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === "employees") {
        ...
        // Filter out student role users as requested: "employee management should not contain redundant/unnecessary/wrong info"
        const employeesOnly = (data || []).filter((u: any) => u.role === "hr" || u.role === "recruiter");
        setUsersList(employeesOnly);
  ...
  }, [session, isPending, activeTab, router]);
  ...
  const startScreening = async () => {
    ...
    try {
      ...
      if (!uiResults.length) {
        throw new Error("Screening timed out.");
      }
  ...
  const handleInitiateOnboarding = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!onboardingForm.candidate_email || !onboardingForm.role || !onboardingForm.start_date || !onboardingForm.salary) {
      return alert("Please fill all onboarding fields.");
    }
    setOnboardingSubmitting(true);
    try {
      await api.post("/onboarding/initiate", onboardingForm);
  ...
  const username = displayName || currentUser.name || currentUser.username || currentUser.email || "Admin";
    ...
    <div className="min-h-screen bg-[#f5f7fb] flex flex-col pt-20">
      ...
      <div className="v2-header-full">
        <div className="flex items-center gap-4">
          {/* Support avatar photo from preferences in TopBar */}
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center overflow-hidden border-2 border-white shadow-md">
            {prefs.avatar ? (
              <img src={prefs.avatar} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-bold uppercase">System Control</p>
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-6">
          ...
          <NotificationCenter prefs={prefs.notifications} />
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 text-xs font-black uppercase tracking-widest transition-all"
          >
        ...
        <div className="v2-sidebar pt-6">
          <div className="flex flex-col gap-1">
            <button
              ...
              className={`v2-nav-item ${activeTab === "screening" ? "active" : ""}`}
            >
            ...
            <button
              ...
              className={`v2-nav-item ${activeTab === "interviews" ? "active" : ""}`}
            >
            ...
            <button
              ...
              className={`v2-nav-item ${activeTab === "onboarding" ? "active" : ""}`}
            >
            ...
            <button
              ...
              className={`v2-nav-item ${activeTab === "employees" ? "active" : ""}`}
            >
          ...
          <div className="v2-content-container">
            {activeTab !== "settings" && (
              <div className="mb-10 flex items-center justify-between">
                ...
                {activeTab !== "screening" && activeTab !== "interviews" && (
                  <button
                    onClick={fetchData}
                    disabled={loading}
                    className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-white border border-gray-100 shadow-sm text-xs font-black uppercase tracking-widest text-blue-600 hover:bg-gray-50 transition-all"
                  >
            ...
            {/* TAB: SETTINGS */}
            {activeTab === "settings" && (
              <SettingsPage user={{ username, email: currentUser.email, role: currentUser.role }} />
            )}
                  ...
                  <div className="space-y-5 animate-fade-up">
                    ...
                    <div className="glass-card p-8 space-y-6 !rounded-[2.5rem] border-white/40 shadow-xl">
                      <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em]">
                        ...
                        {/* No loading spinner on request JD */}
            ...
            {activeTab === "onboarding" && !loading && (
              <div className="animate-fade-up space-y-6">
                <div className="flex justify-between items-center gap-4 flex-wrap">
                  <div className="glass-card p-6 flex-1 !rounded-[2rem] border-white/40">
                    <p className="text-sm font-black text-gray-900">Active Employee Onboarding</p>
                    <p className="text-xs text-gray-500 mt-1">Manage onboarding checklists, send offer letters, and track joining progress.</p>
                  </div>
                  <button
                    onClick={() => setIsOnboardingModalOpen(true)}
                    className="flex items-center gap-2 px-6 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all hover:scale-[1.02]"
                  >
                ...
                {onboardingTasks.length > 0 ? (
                  <div className="glass-card overflow-hidden !rounded-[2.5rem] border-white/40 shadow-2xl">
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-left">
                        <thead>
                          <tr className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-400 bg-white/5 border-b border-white/10">
                            {["Candidate", "Email", "Status", "Initiated On", "Offer Letter"].map((h) => (
                        ...
                        <tbody>
                          {onboardingTasks.map((task) => (
                            <tr key={task.id} className="table-row-premium">
                              <td className="px-8 py-6 font-bold text-gray-900">{task.candidate_name || "Candidate"}</td>
                              <td className="px-8 py-6 text-gray-500">{task.candidate_email}</td>
                              <td className="px-8 py-6">
                              ...
                              <td className="px-8 py-6">
                                <button
                                  onClick={() => {
                                    setSelectedOfferLetter(task.offer_letter_text);
                                    setIsOfferLetterModalOpen(true);
                                  }}
                                  className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 hover:underline font-bold uppercase tracking-wider"
                                >
                  ...
                  <div className="glass-card p-16 text-center space-y-3 !rounded-[2.5rem] border-white/40">
                    <Rocket className="w-12 h-12 mx-auto text-gray-300" />
                    <p className="text-gray-500 font-medium">No candidates are currently in the onboarding pipeline.</p>
                  </div>
                )}
              </div>
            )}
                    ...
                    <tbody>
                      {usersList.map((user) => (
                        <tr key={user.id} className="table-row-premium">
                          <td className="px-8 py-6 font-bold text-gray-900">{user.username}</td>
                          ...
                          <td className="px-8 py-6">
                            {user.id !== currentUser.id ? (
                              <select
                                ...
                                className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5 text-xs font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                              >
                              ...
                              <span className="px-3 py-1.5 rounded-xl bg-gray-100 border border-gray-200 text-xs font-bold text-gray-600 uppercase">
                                {user.role}
                              </span>
                            )}
                          </td>
      ...
      {/* MODAL: Initiate Onboarding Form */}
      {isOnboardingModalOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          />
          <div className="relative w-full max-w-md bg-white rounded-[2rem] p-8 shadow-2xl flex flex-col overflow-hidden animate-fade-up text-gray-900 border border-gray-100">
            <div className="flex justify-between items-center pb-4 border-b border-gray-100">
              <h3 className="text-xl font-black text-gray-900 tracking-tight">Initiate Onboarding</h3>
              <button
                onClick={() => setIsOnboardingModalOpen(false)}
            ...
            <form onSubmit={handleInitiateOnboarding} className="space-y-5 mt-5">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Candidate Email</label>
                <input
                  type="email"
                  required
                  placeholder="name@example.com"
                  value={onboardingForm.candidate_email}
                  onChange={(e) => setOnboardingForm({ ...onboardingForm, candidate_email: e.target.value })}
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-sm font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Salary Package</label>
                <input
                  type="text"
                  required
                  placeholder="$80,000 / year"
                  value={onboardingForm.salary}
                  onChange={(e) => setOnboardingForm({ ...onboardingForm, salary: e.target.value })}
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-sm font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-lg flex items-center justify-center gap-2 mt-4"
              >
      ...
      {isOfferLetterModalOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          />
          <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] p-8 shadow-2xl flex flex-col overflow-hidden animate-fade-up text-gray-900 border border-gray-100 max-h-[80vh]">
            <div className="flex justify-between items-center pb-4 border-b border-gray-100 flex-shrink-0">
              <h3 className="text-xl font-black text-gray-900 tracking-tight">Generated Offer Letter</h3>
              <button
                onClick={() => setIsOfferLetterModalOpen(false)}
                className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600"
              >
            ...
            <div className="overflow-y-auto my-6 p-6 bg-gray-50 rounded-2xl border border-gray-100 flex-1 whitespace-pre-line text-sm text-gray-700 leading-relaxed font-mono">
              {selectedOfferLetter}
            </div>
            <button
              onClick={() => setIsOfferLetterModalOpen(false)}
              className="w-full py-3.5 bg-gray-900 hover:bg-gray-800 text-white font-black text-xs uppercase tracking-widest rounded-2xl flex-shrink-0"
            >
              Close Window
            </button>
          </div>
